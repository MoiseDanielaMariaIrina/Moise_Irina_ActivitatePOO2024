...
public:
	...
	/*() alocam un obiect iar [] alocam un vector de obiecte?*/

	ComplexComercial(Magazin mag, Cofetarie* cof, int nrMasini, Masina* masini) {
		this->magazin = mag;
		this->cofetarie = new Cofetarie(*cof);//*cof asa obtinem obiectul de la pointer?
	...
	friend ostream& operator<<(ostream& o, const ComplexComercial& cc) {
		...
		o << *cc.cofetarie << endl; /*punem cu * ca sa dereferentiem pt ca, cofetarie este pointer */
	...
	ComplexComercial operator--(int) {
		...
		--(*this); /*am apelat din operator-- de mai sus*/
		return cc;
	}

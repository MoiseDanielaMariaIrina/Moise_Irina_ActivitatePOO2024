...
public:
	...
	string& operator[](int index) {
		if (index >= 0 && index < this->nrRoti) {
			...
			throw "Indexul este in afara limitelor."; //ne arunca o exceptie
			//throw
			//try
			//catch
		}
	...
	friend istream& operator>>(istream& input, Masina& m) //facem cu referinta ca sa putem sa scriem??
	{
		cout << "Marca:";
		input >> m.marca;
		cout << "Nr roti:";
...
int main() {
	...
	try {
		cout << m2[1] << endl;
		m2[1] = "Hankook"; //trb sa transmitem prin referinta, deci punem & la string&operator[]
		cout << m2[1] << endl;
	}
	catch (int ex) {
